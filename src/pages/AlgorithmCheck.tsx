...
export default function AlgorithmCheck() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-bold text-[#E7E9EA]">Algoritma Kontrolü</h1>
        <p className="text-sm text-[#71767B] mt-1">Tweet taslağını yapıştır, X algoritması + kampanya kurallarına göre analiz et.</p>
      </div>

      <div className="space-y-5">
        <div className="card p-6">
          <label className="text-[10px] font-bold text-[#71767B] tracking-wider block mb-3">TWEET TASLAGI</label>
          <textarea
            value={text}
          ...
          <div className="flex items-center justify-between mt-3">
            <span className={`text-xs ${text.length > 280 ? 'text-[#F91880]' : text.length > 250 ? 'text-[#FFD400]' : 'text-[#71767B]'}`}>
              {text.length}/280
            </span>
        ...
        {hasText ? (
          <>
            ...
            <div className={`card rounded-2xl p-6 ${algoResult ? (algoResult.passed ? getScoreBg(100) : getScoreBg(algoScore)) : 'bg-[rgba(231,233,234,0.03)]'}`}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  <label className="text-[10px] font-bold text-[#71767B] tracking-wider">ALGORITMA SKORU</label>
                  <div className="text-[10px] text-[#71767B] mt-0.5">Xquik — 11 kontrol (canlı)</div>
                </div>
                {algoLoading ? (
              ...
              {algoResult?.topSuggestion && !algoResult.passed && (
                <div className="text-xs text-[#FFD400] mt-2">{algoResult.topSuggestion}</div>
              )}
            </div>
            ...
            {algoResult && (
              <div className="card p-6">
                <label className="text-[10px] font-bold text-[#71767B] tracking-wider block mb-4">ALGORITMA DETAY</label>
                <div className="space-y-2">
                  {algoResult.checklist.map((check, i) => (
                    <div key={i} className={`flex items-start gap-3 text-sm p-2.5 rounded-lg ${check.passed ? 'bg-[#00BA7C]/5' : 'bg-[#F91880]/5'}`}>
                      <span className={`mt-0.5 text-xs flex-shrink-0 font-bold ${check.passed ? 'text-[#00BA7C]' : 'text-[#F91880]'}`}>
                        {check.passed ? '✓' : '✕'}
                      </span>
            ...
            <div className={`card rounded-2xl p-5 ${getScoreBg(campaignAnalysis.score)}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <div className="text-[10px] text-[#71767B] mt-0.5">Marka kimliği kontrolleri</div>
                </div>
          ...
          <div className="card rounded-2xl p-16 text-center">
            ...
            <div className="text-[#71767B] text-sm font-medium">Tweet metnini yukarıya yaz veya yapıştır.</div>
            <div className="text-[#71767B] text-xs mt-2">Xquik canlı 11 kontrol + kampanya uyum analizi yapılacak.</div>
          </div>
        )}
      </div>
    </div>
