...
export default function AlgorithmCheck() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-bold text-x-text-primary">Algoritma Kontrolü</h1>
        <p className="text-sm text-x-text-secondary mt-1">Tweet taslağını yapıştır, X algoritması + kampanya kurallarına göre analiz et.</p>
      </div>

      <div className="space-y-5">
        <div className="card p-6">
          <label className="text-[10px] font-bold text-x-text-secondary tracking-wider block mb-3">TWEET TASLAGI</label>
          <textarea
            value={text}
          ...
          <div className="flex items-center justify-between mt-3">
            <span className={`text-xs ${text.length > 280 ? 'text-x-accent' : text.length > 250 ? 'text-campaign-gold' : 'text-x-text-secondary'}`}>
              {text.length}/280
            </span>
        ...
        {hasText ? (
          <>
            ...
            <div className={`card rounded-none p-6 ${algoResult ? (algoResult.passed ? getScoreBg(100) : getScoreBg(algoScore)) : 'bg-x-surface-hover'}`}>
              <div className="flex items-center justify-between mb-2">
                <div>
                  <label className="text-[10px] font-bold text-x-text-secondary tracking-wider">ALGORITMA SKORU</label>
                  <div className="text-[10px] text-x-text-secondary mt-0.5">Xquik — 11 kontrol (canlı)</div>
                </div>
                {algoLoading ? (
              ...
              {algoResult?.topSuggestion && !algoResult.passed && (
                <div className="text-xs text-campaign-gold mt-2">{algoResult.topSuggestion}</div>
              )}
            </div>
            ...
            <div className={`card rounded-none p-5 ${getScoreBg(campaignAnalysis.score)}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <div className="text-[10px] text-x-text-secondary mt-0.5">Marka kimliği kontrolleri</div>
                </div>
          ...
          <div className="card rounded-none p-16 text-center">
            ...
            <div className="text-x-text-secondary text-sm font-medium">Tweet metnini yukarıya yaz veya yapıştır.</div>
            <div className="text-x-text-secondary text-xs mt-2">Xquik canlı 11 kontrol + kampanya uyum analizi yapılacak.</div>
          </div>
        )}
      </div>
    </div>
